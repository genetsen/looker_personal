...
Updated: `2026-02-12 19:43:31 EST`
Primary workspace: `/Users/eugenetsenter/Looker_clonedRepo/looker_personal`

## Headlines

1. **DCM + UTM enrichment hardening** - You were tightening how DCM rows get UTM fields, with a constrained fallback for specific campaigns. Next: Run a null-UTM health check for scoped campaigns.
2. **Basis UTM merge and FY26 Q1 backfill** - You were working on Basis UTM union/backfill logic and a patched SQL flow in `/tmp`. Next: Validate patched SQL once, then decide whether to move it into repo.
3. **Offline sheet daily sync pipeline** - You were maintaining the sheet-to-BigQuery sync used for `mft_offline` updates. Next: Verify scheduled-query status and latest rows.

## Details

### 1) DCM + UTM enrichment hardening
What this means: You were tightening how DCM rows get UTM fields, with a constrained fallback for specific campaigns.

Why it matters: This prevents null UTM fields and keeps reporting rows attributed correctly.

Next step now: Run a null-UTM health check for scoped campaigns
...
- `looker-studio-pro-452620.landing.basis_utms_pivoted_flight4_1`

</details>

### 2) Basis UTM merge and FY26 Q1 backfill
What this means: You were working on Basis UTM union/backfill logic and a patched SQL flow in `/tmp`.

Why it matters: This controls whether Basis rows join cleanly into the shared UTM layer without gaps or duplicates.

Next step now: Validate patched SQL once, then decide whether to move it into repo
...
- `looker-studio-pro-452620.landing.basis_utms_pivoted_flight3_2`
- `looker-studio-pro-452620.landing.basis_utms_pivoted_flight4_1`
- `looker-studio-pro-452620.landing.basis_utms_pivoted_flight4_2`
...
Why it matters: If this schedule fails, offline metrics drift from the source sheet and dashboards go stale.

Next step now: Verify scheduled-query status and latest rows
