...
name: dataclaw
description: >
  Export Codex and Codex conversation history to Hugging Face as structured training data.
...
dataclaw prep --source both                # Codex + Codex sessions
...
dataclaw list --source both                # List Codex + Codex projects
...
dataclaw config --source both              # REQUIRED source scope: Codex|codex|both
...
dataclaw export --source both --no-push    # Export Codex + Codex sessions
...
dataclaw update-skill Codex               # Install/update the dataclaw skill for Codex
```
...
- **Source selection is REQUIRED before export** — explicitly set `dataclaw config --source Codex|codex|both` (or pass `--source ...` on export).
