...
successful_files <- phase2_results %>% filter(status == "success")
  ...
  tryCatch({
    # Always read live row data for Phase 5. Spreadsheet formulas/imports can
    # change visible rows without a reliable Drive modifiedTime bump, so cached
    # raw_data is not safe to reuse for production ingestion.
