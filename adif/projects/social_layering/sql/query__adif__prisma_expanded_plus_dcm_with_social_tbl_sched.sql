-- # * SECTION [1]: SOCIAL TEMP TABLE SETUP
--   Clone base ADIF schema into a temp table so social rows can be inserted by subset column mapping.
  CREATE TEMP TABLE social_rows AS
  SELECT
    *
...
-- # * SECTION [2]: SOCIAL INSERT
--   Insert WP social rows with ad_set->package and ad->placement mapping.
  INSERT INTO social_rows (
    package_id_joined,
    date,
    flight_status_flag,
...
-- # * SECTION [2]: SOCIAL DAILY AGG
--   Aggregate the social staging view to ad-level daily grain.
    social_daily AS (
      SELECT
        date_day,
        CASE
      ...
      FROM `repo_stg.stg__adif__social_crossplatform`
      GROUP BY 1,2,3,4,5,6,7,8,9
    ),


-- # * SECTION [3]: TOKEN PREP
...
-- # * SECTION [5]: PACING DEDUPE
--   Keep WP pacing rows with valid budgets and dedupe by adgroup flight window.
    pacing_dedup AS (
      SELECT
        CASE
...
-- # * SECTION [6]: PACING DAILY
--   Expand pacing windows to daily planned spend by date+platform+campaign+ad_set.
    pacing_daily AS (
      SELECT
        day AS date_day,
...
-- # * SECTION [7]: SOCIAL + PLAN JOIN
--   Join social facts to pacing plan via campaign_id/date/platform/ad_set.
    social_joined AS (
      SELECT
        s.*,
...
-- # * SECTION [8]: SOCIAL CORE MAPPING
--   Convert social metrics into ADIF-compatible row-level fields.
    social_mapped AS (
      SELECT
        CONCAT(
          'Social_',
...
-- # * SECTION [9]: PACKAGE ROLLUPS
--   Compute package-level social totals for pacing/over-under fields.
...
-- # * SECTION [10]: SOCIAL ROW OUTPUT
--   Emit social rows into the temp table schema.
  SELECT
    package_id_joined,
    date,
    CASE
...
-- # * SECTION [11]: FINAL UNION OUTPUT
--   Output base ADIF rows plus appended social rows for scheduled query destination write.
  SELECT
    *
  FROM `repo_stg.adif__prisma_expanded_plus_dcm_updated_fpd_view`
  UNION ALL
