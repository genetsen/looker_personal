...
--   Insert WP social rows at date+campaign+platform grain with campaign-ID pacing budget joins.
...
--   Aggregate the social staging view to date+platform+campaign grain.
    social_daily AS (
      SELECT
        date_day,
        social_platform,
...
-- # * SECTION [4]: PACING DAILY
--   Expand pacing windows to daily planned spend by date+platform+campaign.
    pacing_daily AS (
      SELECT
        day AS date_day,
        social_platform,
...
-- # * SECTION [5]: SOCIAL + PLAN JOIN
--   Join social facts to pacing plan via campaign_id/date/platform only.
    social_joined AS (
      SELECT
        s.*,
