...
2. Reads DCM, original FPD, updated FPD, and normalized MIQ Polaris Email rows with `date >= 2025-01-01`.
3. Removes the ADIF-only advertiser, source-file, and account filters used by the ADIF production pipeline.
4. Uses Polaris Email for each mapped package inside its loaded minimum-to-maximum date range, while keeping original and updated FPD outside that range. FPD landing evidence is not deleted.
...
8. Adds Ritual Amazon Ads rows from the runner-maintained landing table, mapping total cost, impressions, clicks, video starts, and video completions into generic delivery metrics while preserving both total cost and supply cost with the other Amazon report fields in `amzn_*` columns.
...
- [Polaris Email package mapping](https://console.cloud.google.com/bigquery?project=looker-studio-pro-452620&p=looker-studio-pro-452620&d=landing&t=polaris_email_package_mapping&page=table) - active feed/platform/campaign/ad-group mappings for MIQ's Polaris Email path.
- [Polaris Email daily delivery](https://console.cloud.google.com/bigquery?project=looker-studio-pro-452620&p=looker-studio-pro-452620&d=landing&t=polaris_email_delivery_daily&page=table) - normalized package/date/platform/campaign/ad-group/ad metrics plus raw values and source lineage.
- [Apollo DCM creative-image asset map](https://console.cloud.google.com/bigquery?project=looker-studio-pro-452620&p=looker-studio-pro-452620&d=landing&t=apo_dcm_creative_image_asset_map&page=table) - a controlled map built from the [Creative Assignment workbook](https://docs.google.com/spreadsheets/d/1U46ZJ4U6XCLTXqtNXlun7uY0L1iyG88RZ6HMiOeACyU/edit). Its `STEP 1 | INPUT - Creative Details` `Asset Name` becomes the normalized DCM creative name; V3 removes only a final size suffix from Apollo DCM `creative` values before matching. The DCM branch [loader](model/branches/dcm/load_apo_dcm_creative_image_map.R) retains every source asset with a publication status and uses the [atomic GitHub publisher](model/branches/dcm/publish_github_assets.R) to publish supported images directly to `genetsen/apo-db-creat` `main`, without a local media checkout. Existing paths remain unchanged, identical files are no-ops, concurrent branch changes stop the run, and BigQuery is replaced only after every intended path is confirmed on `main`. Unavailable files and non-image sources remain blank rather than inferred.
...
- [Legacy-schema RTL compatibility table](https://console.cloud.google.com/bigquery?project=looker-studio-pro-452620&p=looker-studio-pro-452620&d=landing&t=rtl_conv_report&page=table) - the 18-column source for the existing Ritual Omni conversion dashboard. The universal runner refreshes it from direct CM360 history immediately after the upstream merge, with unavailable delivery-only and Sheet-lineage fields left blank. V3 does not read it, and the canonical V3 builder contains no dormant Sheet-conversion branch.
...
- [Master evidence model v3](https://console.cloud.google.com/bigquery?project=looker-studio-pro-452620&p=looker-studio-pro-452620&d=master_stg&t=data_model_v3&page=table) - **Current production master model.** Clustered by `_advertiser`, it preserves source rows at their natural grain, including MIQ Polaris Email at package/date/platform/campaign/ad-group/ad grain with explicit `polaris_*` lineage. It carries summable `_planned_*` metrics once per package/date, keeps repeated plan context in `qa_v3_package_planned_*_doNotSum`, and joins direct CM360 RTL conversions at package/date/placement/creative detail. Planned-only TV, Print, OOH, and dOOH rows use planned spend and impressions as delivered values when planned impressions are available, while real delivery still takes precedence. Conversion-only evidence rows keep conversion fields but no delivery metrics. The universal runner's `Master Data Model Clustered Advertiser Refresh` step rebuilds and verifies it after refreshing the compatibility support table.
- [Ritual dashboard compatibility view](https://console.cloud.google.com/bigquery?project=looker-studio-pro-452620&p=looker-studio-pro-452620&d=master_stg&t=ritual_data_model&page=table) - Ritual-only projection over `data_model_v3` that preserves the established dashboard names and Omni topic while exposing v3's wider schema. It currently has 242 fields: 125 compatibility fields, all 116 v3 fields that were previously hidden, and a friendly `creative_name` alias for canonical `_creative_name`. Both creative fields remain available, while Omni presents the alias as `Creative Name`. The view now includes `amzn_total_cost`, and Amazon `final_spend` uses that value while `amzn_supply_cost` remains available as source evidence. Its wildcard exclusion contract carries future non-conflicting v3 fields after the view is recreated to register an expanded source schema. The [maintained Ritual view builder](model/reporting_outputs/create_master_stg_ritual_data_model.sql) is the deployment source, and its [SQL Change Guard manifest](model/reporting_outputs/ritual_data_model_v3_schema_expansion.qa.json) protects existing rows, measures, derived rates, dimensions, the creative alias, and the Ritual-only filter.
- [Master evidence model](https://console.cloud.google.com/bigquery?project=looker-studio-pro-452620&p=looker-studio-pro-452620&d=master_stg&t=data_model&page=table) - Live package/date compatibility base that supplies stable context to v3. Original FPD `views` and `completed_views` feed final `_video_views` and `_video_comps`; do not use it as the default for new master-model work.
...
- Amazon Ads source fields are preserved in `amzn_*` fields. The model maps Amazon total cost into `_spend`, and maps impressions, clicks, video starts, and complete video views into the generic delivery metric fields. Amazon supply cost, sales, purchases, units sold, branded searches, source email metadata, and source file metadata stay visible in the Amazon-specific columns.
- Amazon Ads rows use `qa_media_data_type = 'amazon_ads'` and `qa_row_data_source_primary = 'amazon_ads'`. Amazon `sales` is not mapped into `_spend` because it is outcome revenue, not media spend; `total_cost` is preserved as `amzn_total_cost` and mapped into `_spend`, while `supply_cost` remains separately available as `amzn_supply_cost`.
