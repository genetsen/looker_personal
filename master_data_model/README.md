...
10. Standardizes `_advertiser` and `_advertiser_name` to the same canonical value using the separate advertiser mapping table. Unmapped clients default to Prisma's advertiser name after removing legal suffixes such as `Inc`, `LLC`, `Corp`, and `Ltd`; `_advertiser_short_name` remains available for source lineage.
...
- `_creative_name` is the package/date model's canonical creative label. It uses `fpd_orig_creative`, then `amzn_ad_name`, then `s_creative_name`; Amazon is checked before the shared social field because Amazon rows currently carry their ad name in both fields. DCM creative names are intentionally excluded because they can be one-to-many at package/date grain and remain available in `data_model_delivery_detail_v2`.
...
- Manual evidence is visible in `man_*` fields, including edit ID, reason, loader account, source sheet URL, modified time, row-level edit audit fields, daily values, and total replacement values marked with `_doNotSum` where they are repeated across daily rows. Friendly `qa_manual_*` fields expose whether a row has a manual edit, when the user edited it when available, who edited it when Google exposes that identity, and when the edit was accepted into BigQuery.
...
- `_advertiser` and `_advertiser_name` are the same canonical advertiser value. The [advertiser mapping](https://console.cloud.google.com/bigquery?project=looker-studio-pro-452620&p=looker-studio-pro-452620&d=master_stg&t=advertiser_mapping&page=table) groups known source-name and short-code variants. When a new client has no mapping row, the model uses Prisma's advertiser name and removes trailing legal suffixes such as `Inc`, `LLC`, `Corp`, and `Ltd`. `_advertiser_short_name` remains unchanged for source lineage.
