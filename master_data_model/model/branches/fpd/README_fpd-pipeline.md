...
| [Polaris FPD preview](polaris/README.md) | Platform/date/campaign/ad group/ad; package/Sunday-start week for overlap review | Locally normalized Meta and TikTok delivery, approved MIQ package candidates, reconciliation, and weekly current-FPD overlap evidence. | QA-only Stage 1 workflow. It preserves Polaris's daily grain but compares it with MIQ's native weekly FPD grain. It does not publish a landing table, replace legacy FPD, edit a mapping Sheet, or feed the model. |
