...
| Prototype duplicate-name fix | Changed the temp copy's requested-package dropdown source from short package names to `short package | full package key`, then selected two `DealBook` rows. | The two selected `DealBook` rows resolved to distinct full package keys and package IDs: `P3FCQTS` and `P3CGHZG`. | The simplest safe fix is to make the dropdown value unique and resolve rows from a stable package key, not from the duplicated short label. |
...
| Short package labels are not always unique. | In the temp copy, selecting `DealBook` populated the first matching full package key even though `DealBook` appears multiple times in the requested-package list. A later temp-copy prototype using `short package | full package key` let two `DealBook` selections resolve to different package IDs. | Make the partner-facing package selector unique by including a stable package key or package ID in the dropdown label. Avoid resolving rows from short package name alone. |
| Inserted or newly launched package rows can push manual ranges out of alignment. | The template has formula-owned package lists, user-editable checkbox selections, mapping columns, and helper ranges next to each other on `Config`. | Keep selectable/mapping logic keyed by stable package key. Prefer formulas that spill from the current package list over fixed row-by-row manual alignment, and regenerate dropdown sources from checked rows instead of maintaining a copied list by hand. |
