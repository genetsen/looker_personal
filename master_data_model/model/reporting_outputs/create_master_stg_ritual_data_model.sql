-- Purpose: Maintain the Ritual-only BigQuery compatibility view used by Omni.
-- Reads: master_stg.data_model_v3.
-- Produces: master_stg.ritual_data_model with legacy aliases plus all additional v3 fields.
-- Safe use: validate the sibling QA view and SQL Change Guard report before deployment.

CREATE OR REPLACE VIEW `looker-studio-pro-452620.master_stg.ritual_data_model`
OPTIONS (
  description = 'Ritual-only Omni compatibility view over data_model_v3. Preserves legacy dashboard aliases and carries every additional v3 field through the v3 wildcard exclusion contract.'
) AS
SELECT
  -- LIVE VIEW NOTE: Ritual-only compatibility projection over the canonical
  -- package/date model using the consolidated FPD and QA fields.
