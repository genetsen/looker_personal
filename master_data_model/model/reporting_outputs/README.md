...
The production table now uses `product_group_sales_dollars` and `product_group_tdp` instead of the previous `sales_dollars` and `sales_tdp`. It also includes Total Brand and Total Brand Granola sales/TDP benchmarks. See the [dashboard migration guide](/Users/eugenetsenter/Looker_clonedRepo/looker_personal/purely_elizabeth/WEEKLY_SALES_DASHBOARD_MIGRATION.md) for exact definitions, aggregation guidance, and Looker Studio update steps.
