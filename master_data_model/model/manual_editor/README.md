...
The raw manual table is only the current replaceable snapshot. A separate permanent history table records every accepted loader state before the raw snapshot is replaced, so an accepted edit can be recovered even after later refreshes. During a refresh, generated rows from the rewritten `Package Editor` do not override or duplicate trusted prior manual state unless a source-backed row has real user edit evidence: `Manual Edit At` or `Manual Edit By`, or trusted prior accepted history. `Manual Edit Published At` alone on a loose sheet row is loader output, not proof of a new user edit. Before writing, the loader clears values through the current editor grid's final row while preserving formatting, so old rows cannot remain visible below the newly written source-authoritative rows. Before any BigQuery table replacement or visible sheet rewrite, the loader compares the last accepted manual rows to the proposed upload and stops if a prior accepted edit would go missing, become inactive or blocked, or lose an edited field without a newer user audit stamp. This keeps the visible interface the same while preventing blank or stale output rows from deleting, splitting, or inventing user edits.
...
Started new rows highlight the specific missing required cells or invalid date cells. Planned cells turn red only when a planned edit uses a partial date range instead of the full visible flight dates. Orange means a meaningful normalized difference from the current source baseline; values already backed by validated manual updates turn purple. The loader also blocks invalid rows backend-side, so red frontend feedback is a warning to fix the row before requesting a refresh.
