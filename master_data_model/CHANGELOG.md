...
**Creative-image behavior is now written down instead of re-derived** ([Markdown](/Users/eugenetsenter/Looker_clonedRepo/looker_personal/master_data_model/model/branches/dcm/README_dcm-pipeline.md)) — 🔵 **Verified with unfinished work**<br>The [DCM pipeline guide](/Users/eugenetsenter/Looker_clonedRepo/looker_personal/master_data_model/model/branches/dcm/README_dcm-pipeline.md) now states how creative images actually reach the model: the supported refresh entrypoint and its safety gates, the exact creative-name match rule, which field a report should read, and why most Apollo creatives legitimately have no image. It names the four creatives whose names cannot match today and explains the single reason they fail, so low image coverage is recognized as a source-data condition rather than investigated as a pipeline fault. Two check queries are included so anyone can confirm current coverage instead of relying on documented counts. The pipeline itself is unchanged.

- **⚠️ Unfinished** - Commit the guide update.
